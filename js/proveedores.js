...
function renderProveedores() {
    ...
    proveedores.forEach(function(p) {
        ...
        fila.innerHTML =
            ...
            '<td data-label="Registro">' + fecha + '</td>' +
            '<td data-label="Acciones">' +
                '<button onclick="editarProveedor(' + p.id + ')">✏️</button>' +
                '<button onclick="eliminarProveedor(' + p.id + ')" style="background:#e74c3c;margin-left:6px">❌</button>' +
            '</td>'
...
openModalBtn.addEventListener("click", () => {
    ...
    limpiarForm()
    modal.classList.add("active") // 🔥 Cambiado
})
